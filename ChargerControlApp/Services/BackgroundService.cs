        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                try
                {
                    ...
                    {
                        if (_hardwareManager.Charger.GetCachedVoltage() >= 10)
                        {
                    ...
                    {
                        if (_hardwareManager.Charger.GetCachedVoltage() >= 57.5 &&
                            _hardwareManager.Charger.GetCachedCurrent() < 2.5)
                        {
                            _chargingStationStateMachine.TransitionTo<OccupiedState>();
                        }
                    }
            ...
            _grpcHost = Host.CreateDefaultBuilder()
                .ConfigureWebHostDefaults(webBuilder =>
                {
                    ...
                    webBuilder.ConfigureServices(services =>
                    {
                        ...
                        services.AddSingleton(provider =>
                            _mainProvider.GetRequiredService<ChargerActionServiceImpl>());
                    });
